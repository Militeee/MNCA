...
def run_experiment(sample_idx, samples, labels, class_names, output_dir):
    ...
    # Save models
    torch.save(model.state_dict(), os.path.join(experiment_dir, 'standard_model.pt'))
    torch.save(model_mix.state_dict(), os.path.join(experiment_dir, 'mixture_model.pt'))
    torch.save(model_gmix.state_dict(), os.path.join(experiment_dir, 'mixture_model_noise.pt'))
    ...
    with open(losses_path, 'w') as f:
        f.write('class_name,standard_final_loss,mixture_final_loss,stochastic_final_loss\n')
        f.write(f"{class_name},{final_losses['standard_final_loss']:.6f},"
